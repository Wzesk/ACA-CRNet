...
import numpy as np
from dataset.rice2_data_loader import dataloader
import logging
import datetime
...
logging.basicConfig(level = logging.CRITICAL,
    ...
    filename = 'train_ACA_CR_Rice2_log_'+now+'.txt',
    filemode = 'w')
...
def train(config):
    ...
    if config.use_gpu == False and t.cuda.is_available() == True:
        ...
        print("NVIDIA GPU {} for Training".format(t.cuda.get_device_name()))
            ...
            if iteration % config.show_freq == 0:                
                 with t.no_grad():
                     ...
                     logging.critical("epoch[{}]({}/{}):loss_fake:{:.8f}".format(
                          epoch,iteration,len(train_dataloader),loss.item()))
                     #valid step
                     inputdata,img_csm,s2img,patch_path_out=next(val_dataiter)
                     ...
                     save_result_img(s2img1, os.path.join(config.output_dir+'/val_img', "{}_truth_iteration.jpg".format(iteration)))
                     save_result_img(fake_img, os.path.join(config.output_dir+'/val_img', "{}_fake_iteration.jpg".format(iteration)))
                     save_result_img(img_csm, os.path.join(config.output_dir + '/val_img',
                                                            "{}_smask_iteration.jpg".format(iteration)))
